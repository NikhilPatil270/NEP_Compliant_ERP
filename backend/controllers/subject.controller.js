...
const getSubjectController = async (req, res) => {
  try {
    const { branch, class: classNum } = req.query;
    let query = {};
...
const addSubjectController = async (req, res) => {
  const { name, code, branch, class: classNum, credits } = req.body;

  if (!name || !code || !branch || !classNum || !credits) {
...
const updateSubjectController = async (req, res) => {
  ...
  if (classNum) updateFields.class = classNum;
